...
var get_js_file = exports.get_js_file = function (config, _req) {
  ...
  var socket = "";
  var selectors = _.filter (config.sites.selectors, function (sel) {return sel.page === _req.url;});
  if (selectors.length > 0) {
    ...
    var gm_object = "var ghostmill={socket:io.connect('"+conf.server.base_url+"'),send:function(sel){if("+config.static.debug_mode+"===true)console.log('send');this.socket.emit('event',{'type':'tick','ip': '"+_req.ip+"', 'url': '"+_req.url+"', 'sel':sel.sel,'event':sel.e});}};";
