...
var get_js_file = exports.get_js_file = function (config, _req) {
  ...
  var _listeners = (selectors).reduce (function (memo, sel) {
    return memo + 
      "$('"+sel.sel+"').on('"+sel.event+"',function(){console.log('event');ghostmill_socket.emit('event',{'type':'tick','ip': '"+_req.ip+"', 'url': '"+_req.url+"', 'sel':'"+sel.sel+"','event':'"+sel.event+"'})});";
  }, "");
